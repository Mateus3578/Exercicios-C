...
/* ============================================================================================= */
// Trecho que usa pilhas
/* ============================================================================================= */

/* Cria uma pilha. */
Pilha_Tree *pilha_tree_cria(){
	Pilha_Tree *p=(Pilha_Tree *)malloc(sizeof(Pilha_Tree));
	p->prim=NULL;
	return p;
}
...
void pilha_tree_push(Pilha_Tree *p, Tree *v){
	Lista_Tree *l=(Lista_Tree *)malloc(sizeof(Lista_Tree));
	l->info=v;
	l->prox=p->prim;
	p->prim=l;
}
...
/* Monta uma expressão aritimética usando uma árvore binária. */
Tree *insere_expressao(Tree *arvore, char *expressao){
    Tree *no;
    Pilha_Tree *pilha = pilha_tree_cria();
    ...
    for(int i = 0; expressao[i] != '\0'; i++){
        ...
        if(isalnum(expressao[i])){
            pilha_tree_push(pilha, no);
        } else {
            no->dir = pilha_tree_pop(pilha);
            no->esq = pilha_tree_pop(pilha);
            pilha_tree_push(pilha, no);
        }
    }
    
    Tree *raiz = pilha_tree_pop(pilha);  
    pilha_tree_libera(pilha);
    return raiz;
}

/* Escreve a versão infixa/simétrica dos dados de uma árvore, na mesma linha */
void escreveNoArquivoTextoInfixa(Tree *arvore, FILE *arquivo){    	
    if(!vazia(arvore)){
        escreveNoArquivoTextoInfixa(arvore->esq, arquivo);
        fprintf(arquivo, "%c ", arvore->info);
        escreveNoArquivoTextoInfixa(arvore->dir, arquivo);
    }
}

/* Escreve a versão pósfixa dos dados de uma árvore, na mesma linha */
